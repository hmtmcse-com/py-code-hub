...
def enrole(ip, port=4370, timeout=100):
    zk = ZK(ip, port=port, timeout=timeout)
    conn = zk.connect()
    conn.set_user(uid=5, name='Touhid Mia', privilege=const.USER_DEFAULT, password='12345678', group_id='', user_id='5', card=0)
    zk.enroll_user(uid=5)
    conn.disconnect()
    return ""
...
# out = get_all_attendance_from_device(ip="192.168.1.25", device_id="device1")

enrole(ip="10.0.100.244")
